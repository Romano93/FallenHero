    ...
    internal class Reader
    {
        public static void Run()
        {
            while (true)
            {
                ...
                Structs.LocalPlayer.m_iBase = Memory.ReadMemory<int>(Structs.Base.ClientPointer + signatures.dwLocalPlayer);
                Structs.LocalPlayer.m_iTeamNum = Memory.ReadMemory<int>(Structs.LocalPlayer.m_iBase + Offsets.m_iTeamNum);
                Structs.LocalPlayer.m_iClientState = Memory.ReadMemory<int>(Structs.Base.EnginePointer + signatures.dwClientState);
                Structs.LocalPlayer.m_iGlowBase = Memory.ReadMemory<int>(Structs.Base.ClientPointer + signatures.dwGlowObjectManager);
                ...
                Structs.LocalPlayer.m_hActiveWeapon = Memory.ReadMemory<int>(Structs.LocalPlayer.m_iBase + Offsets.m_hActiveWeapon);

                for (var i = 0; i < 64; i++)
                {
                    var entity = Structs.Arrays.Entity[i];

                    entity.m_iBase = Memory.ReadMemory<int>(Structs.Base.ClientPointer + signatures.dwEntityList + i * 0x10);
                ...
                for (var i = 0; i < 16; i++)
                {
                    ...
                    var WeaponBase = Memory.ReadMemory<int>(Structs.Base.ClientPointer + signatures.dwEntityList + (ActiveWeapon - 1) * 0x10);
